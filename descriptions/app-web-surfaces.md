...
7. **App store** — **phase 1** portal `/store` copy-install **removed**. Catalog
   lives only on the installer agent (`/apps/installer/`). **phase 2:**
   `pnet_installer` desire + status, notify only. **phase 3:**
   `pnet_installer bootstrap` installs pNet + agent from local binaries.
   **phase 3b:** catalog is GitHub URL lists in
   `~/.pnet/installer/app_sources/` (managed `pnet.list` + extra org files);
   store cards from `pnet-app.json` / GitHub API / cache. Signed catalog
   auto-install remains later (`descriptions/app-store-installer.md`).
